...
class tile():
    def __init__(self, x, y, color, points):
        ...
        self.points = points
        self.active = True
        self.size = 30       # Mod - new variable

        if randint(0,80) < 2:       # Randomly set the speed change to 1 or -1
            self.speed = 1              # MOD
            self.color = (128,128,128)  # MOD
        elif randint(0,80) < 2:         # MOD
            self.speed = -1             # MOD
            self.color = (128,128,128)  # MOD
        else:                           # MOD
            self.speed = 0              # Change of speed for this tile
...
# New function **********************************************************
    def updateSpeed (self, ds):            # Change the speed of the ball
        if self.dx > 0:
            self.dx = self.dx + ds
        else:
            self.dx = self.dx - ds
