...
K_Y          = 89
K_Z          = 90
_norml = "abcdefghijklmnopqrstuvwxyz0123456789-=[]\\;',./`"
_shftd = "ABCDEFGHIJKLMNOPQRSTUVWXYZ)!@#$%^&*(-+{}|:\"<>?~"
...
def _keyPressed (k):
    global _user, _keyp, _shift, _norml, _shftd
    if _keyp:
        if k==K_RSHIFT or k==K_LSHIFT:
           _shift = True
        elif _shift:
            c = _norml.find(chr(k))
            kk = _shftd[c]
            _user.keyPressed (ord(kk))
        else:
            _user.keyPressed(k)

def _keyReleased (k):
    global _user, _keyr, _shift, _norml, _shftd
    if _keyr:
        if k==K_RSHIFT or k==K_LSHIFT:
            _shift = False
        elif _shift:
